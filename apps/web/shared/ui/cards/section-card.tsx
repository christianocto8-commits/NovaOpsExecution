...
export function SectionCard({
  ...
  return (
    <section className={cn("min-w-0 overflow-hidden rounded-2xl border border-[#E7ECE9] bg-white shadow-sm", className)}>
      {(title || description || actions) && (
        <div className="flex min-w-0 flex-col justify-between gap-3 border-b border-[#E7ECE9] px-4 py-4 sm:flex-row sm:items-start sm:px-6 sm:py-5">
          <div className="min-w-0">
            {title ? <h2 className="break-words text-base font-semibold text-[#1E1E1E] sm:text-lg">{title}</h2> : null}
