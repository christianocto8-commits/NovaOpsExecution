...
export function WorkflowToolbox() {
  function handleDragStart(event: DragEvent<HTMLButtonElement>, item: WorkflowNodeRegistryItem) {
  ...
  return (
    <aside className="overflow-y-auto border-r border-slate-200 bg-white p-4">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Toolbox</p>
      ...
      <div className="mt-5 space-y-6">
        {groups.map((group) => {
          const items = workflowNodeRegistry.filter((item) => item.group === group.id);
