...
class WorkflowStep(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    step_type: Mapped[WorkflowStepType] = mapped_column(Enum(WorkflowStepType), nullable=False)
...
class WorkflowApproverType(str, enum.Enum):
    role = "role"
    user = "user"
    outlet = "outlet"
...
class WorkflowApprovalMatrix(Base):
    __tablename__ = "workflow_approval_matrices"
    ...
    approver_type: Mapped[WorkflowApproverType] = mapped_column(
...
class WorkflowInstanceStepStatus(str, enum.Enum):
    pending = "pending"
    active = "active"
    approved = "approved"
    rejected = "rejected"
...
class WorkflowInstanceStep(Base):
    ...
    status: Mapped[WorkflowInstanceStepStatus] = mapped_column(
        Enum(WorkflowInstanceStepStatus),
        default=WorkflowInstanceStepStatus.pending,
        nullable=False,
    )
...
class WorkflowTask(Base):
    ...
    instance_step_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
...
class WorkflowApprovalHistory(Base):
    ...
    instance_step_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("workflow_instance_steps.id", ondelete="SET NULL"),
        index=True,
        nullable=True,
    )
...
class WorkflowEscalationRule(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    trigger_after_hours: Mapped[int] = mapped_column(Integer, nullable=False)

    action: Mapped[WorkflowEscalationAction] = mapped_column(
